...
var imports = ["require", "isWhitelisted", "extractHostFromURL", "refreshIconAndContextMenu", "saveChoice", "getAction", "getAllOriginsForTab", "console", "whitelistUrl", "removeFilter", "setupCookieBlocking", "teardownCookieBlocking", "moveCookiesToRealCookieStore", "moveCookiesToFakeCookieStore"];
...
function init()
{
  ...
  $(document).ready(function () {
	$("#startingQuestion").show();
	$("#DontCareResponse").hide();
	$("#SortofCareResponse").hide();
	$("#CareResponse").hide();
	$("#ReallyCareResponse").hide();
	$("#siteControls").show();
	$('#blockedResourcesContainer').hide();
	$("#Nuance_btn").hide();
	$("#predictionInAlphaMessage").hide();
	 
	$('#blockedResourcesContainer').on('change', 'input:radio', updateOrigin);
    $('#blockedResourcesContainer').on('mouseenter', '.tooltip', displayTooltip);
    $('#blockedResourcesContainer').on('mouseleave', '.tooltip', hideTooltip);
    $('#blockedResourcesContainer').on('click', '.userset .honeybadgerPowered', revertDomainControl);
	$('#checkboxG5').on('checked', 'input:checkbox', predictionInAlpha);
  });
...
function predictionInAlpha() {
	$("#startingQuestion").hide();
	$("#siteControls").hide();
	$("#predictionInAlphaMessage").show();
}
...
function refreshPopup(tabId) {
  ...
  if (!origins || origins.length == 0) {
    document.getElementById("blockedResources").innerHTML = "There are no tracking cookies that we see, so our job is done.";
    return;
  }
...
function blockAllDomains() {
  $('.clicker').each(function() {
	var origin = $(this).attr("data-origin");
	console.log("attempting to give secrecy by blocking" + origin);
	var filter = Filter.fromText("||" + origin + "^$third-party");
	FilterStorage.addFilter(filter, FilterStorage.knownSubscriptions["userRed"]);
	$(this).removeClass('block cookieblock noaction').addClass("block");
	$(this).removeClass("data-original-action").addClass('userset')
	settingsDict[origin] = "block";
  })
}
