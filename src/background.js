...
function getOrCreateUuid() {
	function createUuid() {
		// Attribution to this function goes to Byron Salau http://byronsalau.com/blog/how-to-create-a-guid-uuid-in-javascript/
...
function saveChoice(url, choice) {
	var uid = localStorage.uid;
	userChoices.setChoice(url, choice);
	domain = getBaseDomain(extractHostFromURL(url));
	choicesDict = {"one": 1, "two": 2, "three": 3, "four": 4};
	choiceInt = choicesDict[choice];
	userChoices.sendChoiceToServer(uid, domain, choiceInt);
}

// Adds or removes page action icon according to options.
function refreshIconAndContextMenu(tab)
{
  console.log("refreshing icon and context");
  ...
  if(choice) {
	if (choice == "one"){
		...
		chrome.browserAction.setIcon({tabId: tab.id, path: "icons/Prediction-19.png"})
	} else if(choice=="two"){
		...
		chrome.browserAction.setIcon({tabId: tab.id, path: "icons/Prediction-19.png"});
		chrome.notifications.create("VPNreminder", {type: "basic", iconUrl: "/skin/Robot.png", title: "Should you be using a VPN?", message: "It looks like you have some sensitive data on this site. Maybe, you should enable a VPN while you're here?"}, function(){});
	} else {
		chrome.browserAction.setPopup({tabId: tab.id, popup: "skin/popup.html"});
		chrome.browserAction.setIcon({tabId: tab.id, path: "icons/butler-19.png"})
	}
  }; 
